...
if %errorlevel% equ 0 (
    ...
    if %errorlevel% neq 0 (
        ...
        echo Servis durdurulamadı. Tekrar deneniyor...
		goto :stop_any
    ) else (
...
:portable_lic_check
:: portable sürüm için ad.anynet.id kontrolü yapılıyor.
if exist "%APPDATA_ANYDESK%\system.conf" (
    echo %APPDATA_ANYDESK%\system.conf dosyası bulundu.
	goto :portable_lic
) else (
    echo %APPDATA_ANYDESK%\system.conf dosyası bulunamadı. Lütfen AnyDesk yazılımını çalıştırın. 
...
:portable_lic
echo yüklenmiş sürüm için ad.anynet.id kontrolü yapılıyor.
...
echo *********
echo *********
echo Bir yerlerde hata oluştu. İşlemin başarılı olup olmadığı bilinmiyor. Manuel kontrol edin.
echo Eğer conf dosyaları yedeklenmiş ve silinmişse sıfırlanıp ID değişmiştir.
timeout /t 10 /nobreak >NUL
pause
exit /b